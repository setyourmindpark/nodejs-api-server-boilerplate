const mail = reqlib('/app/base/mail');
...
exports.getPI = async (req, res, next) => {
    try {
        ...
        const { recordCountPerPage, paegSize } = conf.config.pagenation;
        const data = {
            where: where,
...
exports.sendMail = async (req, res, next) => {
    try {
        const { to, subject, text } = req.prop;
        await mail.sendMail({ to: to, subject: subject, text: text });
        response.apiResponse(res, {
            code: constant.CODE_SERVICE_PROCESS_1,
        });
    } catch (err) {
...
exports.getPublicKey = (req, res, next) => {
    ...
    // var key = fs.readFileSync(conf.context.rsa.privateKey);
    // var crt = fs.readFileSync(conf.context.rsa.publicKey);
